...
import type { RiderTDF } from '../types/RiderTDF';
...
const riders = ref<RiderTDF[]>([]);
const randomRider = ref<RiderTDF>();
const guesses = ref<RiderTDF[]>([]);
const selectedValue = ref<RiderTDF | null>(null);
...
const baseHeaders = [
  ...
  { key: 'win' },
  { key: 'gt_classic' },
  { key: 'specialties', minWidth: '185px', maxWidth: '185px' },
];
...
const attributes = ['year_tdf', 'jersey', 'nationality', 'win', 'gt_participation', 'classic_participation'];
...
onMounted(async () => {
  if (localStorage.getItem('lastPlayed') !== new Date().toLocaleDateString('en-US', { timeZone: 'Europe/Paris' })) {
    ...
    won.value = JSON.parse(localStorage.getItem('wonTDF') as string);
    guesses.value = JSON.parse(localStorage.getItem('guessesTDF') as string);
  }
  if (won.value) {
    isDialogWinActive.value = true;
  }

  const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/riders-tdf`);
  riders.value = await response.json();
  updateRidersList();
  randomRider.value = (await (await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/riders/random-tdf`)).json())[0];
});

const updateRidersList = () => {
  riders.value = riders.value.filter((rider: RiderTDF) => {
    return !guesses.value.find((guess: RiderTDF) => guess.id === rider.id);
  });
}
 
const selectRider = (riderSelected: RiderTDF | null) => {
  if (!randomRider.value || !riderSelected || guesses.value.find((rider: RiderTDF) => rider.id === riderSelected.id)) return;

  guesses.value.unshift(riderSelected);
  localStorage.setItem('guessesTDF', JSON.stringify(guesses.value));
  ...
  if (riderSelected.id === randomRider.value.id) {
    won.value = true;
    isDialogWinActive.value = true;
    localStorage.setItem('wonTDF', JSON.stringify(true));

    const statsName = 'statsTDF';
    ...
    const colorCounts = guesses.value.flatMap(guess =>
      attributes.map(attr => getAttributeColor(guess[attr as keyof RiderTDF], attr))
    ).reduce((counts, color) => {
      counts[color]++;
      return counts;
...
const getHelpText = (value: string | number, type: string) => {
  switch (type) {
    ...
    case 'year_tdf':
      return getAttributeArrow(value, 'year_tdf') === 'mdi-chevron-up' ? langFile.value.game_datatable_tooltip_year_upper : langFile.value.game_datatable_tooltip_year_lower;
    case 'win':
...
const getAttributeColor = (value: string | number, type: string) => {
  ...
  const valueRider = randomRider.value[type as keyof RiderTDF];
  ...
  const helpGap: any  = {age: 2, year_tdf: 10, win: 5, gt_participation: 1, classic_participation: 2}; // customize variables to return orange to help the player
...
const getAttributeArrow = (value: number|string, type: string) => {
  ...
  if (value < randomRider.value[type as keyof RiderTDF]) return 'mdi-chevron-up';
  if (value > randomRider.value[type as keyof RiderTDF]) return 'mdi-chevron-down';
  if ((!value || !randomRider.value[type as keyof RiderTDF]) && value !== 0) return 'mdi-close';
  return 'mdi-check';
};
...
const formatRiderSpecialities = (rider: RiderTDF) => {
  return {
...
const saveToClipboard = () => {
  ...
  let textToShare = `Cycdle (@Cycdle) TDF#${nb} - ${guesses.value.length} ${langFile.value.game_modal_win_text_share_try}\n`;

  for (const guess of guesses.value.slice().reverse()) {
    for (const key of Object.keys(guess)) {
      if (attributes.includes(key)) {
        const color = getAttributeColor(guess[key as keyof RiderTDF], key);
        let colorEmoji = '';
        if (color === 'green') {
          colorEmoji = '🟩';
...
const customFilter = (_itemTitle: any, query: string, item: any) => {
  ...
  return name.indexOf(query.toLowerCase()) > -1 || nationality.indexOf(query.toLowerCase()) > -1;
}
</script>
      ...
      <template v-slot:item="{ props, item }" >
        ...
        >
          ...
          <template v-slot:append>
            <v-chip>{{item.raw.year_tdf}}</v-chip>
            <img :src="`/src/assets/jersey/${item.raw.jersey}.png`" width="50px" />
          </template>
        </v-list-item>
      </template>
    </v-autocomplete>
    ...
    <v-data-table :items="guesses" :headers="headers" hide-no-data items-per-page="-1" disable-sort>
      ...
      <template v-slot:item.name="{ item }">
        <v-avatar rounded="0" size="100">
          <div :class="['avatar-border', `img-border-${getAttributeColor(item.name, 'name')}`]">
            <img v-if="item.photo" :src="item.photo" id="avatar-img-datatable-tdf" />
            <img v-else src="/src/assets/jersey/unknown.png" id="avatar-img-datatable-tdf" />
          </div>
        </v-avatar>
        <div class="mt-2">{{ item.name.split(' ').splice(0, 2).join(' ') }} <br /> {{ item.name.split(' ').splice(2).join(' ') }}</div>
      </template>
      <template v-slot:item.jersey="{ value }">
        <img :src="`/src/assets/jersey/${value}.png`" width="100px" :class="`border-${getAttributeColor(value, 'jersey')}`"/>
      </template>
      <template v-slot:item.year_tdf="{ value }">
        <v-tooltip :open-on-click="mobile" :location="mobile ? 'bottom' : 'right'" :disabled="getAttributeColor(value, 'year_tdf') === 'green'" max-width="250">
          {{ getHelpText(value, 'year_tdf') }}
          <template v-slot:activator="{ props }">
            <v-chip :color="getAttributeColor(value, 'year_tdf')" v-bind="props">
              {{ value }} <v-icon :icon="getAttributeArrow(value, 'year_tdf')" />
            </v-chip>
          </template>
        </v-tooltip>
      </template>
            ...
            <div class="pixels pt-2">
              <div v-for="guess in guesses.slice().reverse()" :key="guess.id">
                <span v-for="key of Object.keys(guess)" :key="key">
                  <v-icon v-if="attributes.includes(key)" icon="mdi mdi-square" :color="getAttributeColor(guess[key as keyof RiderTDF], key)" />
                </span>
              </div>
            </div>
          </v-card-text>
